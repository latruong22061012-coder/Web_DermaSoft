...
(function() {
    ...
    function renderTable(lichCa) {
        ...
        lichCa.forEach(function(ca) {
            ...
            var badge = '';
            if (ca.TrangThaiDiemDanh == 2) badge = '<span class="admin-badge admin-badge-active"><i class="bi bi-check-circle-fill"></i>Đã điểm danh</span>';
            else if (ca.TrangThaiDiemDanh == 3) badge = '<span class="admin-badge admin-badge-locked"><i class="bi bi-x-circle-fill"></i>Vắng</span>';
            else badge = '<span class="admin-badge" style="background:rgba(255,193,7,.12);color:#856404"><i class="bi bi-clock"></i>Chưa điểm danh</span>';
    ...
    function loadTheoThang(thang) {
        ...
        bacsiFetch('lich-lam-viec?thang=' + thang).then(function(res) {
    ...
    function loadTheoNgay(ngay) {
        ...
        bacsiFetch('lich-lam-viec?ngay=' + ngay).then(function(res) {
            if (res.status !== 200) return;
            updateStats(res.data.thongKe);
            renderTable(res.data.lichCa);
        });
    ...
    // ═══ Nút prev/next ngày & date picker ═══
