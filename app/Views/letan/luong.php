...
<div class="admin-card mb-4">
    ...
    <div class="admin-card-body-flush" style="overflow-x:auto">
        <table class="admin-table">
            ...
            <tbody id="tableLuong">
                <tr><td colspan="13" class="text-center text-muted py-4">Đang tải...</td></tr>
            </tbody>
        </table>
    </div>
</div>
...
(function() {
    ...
    function loadLuong(nam) {
        ...
        letanFetch('luong?nam=' + nam).then(function(res) {
            ...
            if (!data.bangLuong || data.bangLuong.length === 0) {
                tbody.innerHTML = '<tr><td colspan="13" class="text-center text-muted py-4">Chưa có dữ liệu lương</td></tr>';
            } else {
                var html = '';
                data.bangLuong.forEach(function(bl) {
                    ...
                    var loai = bl.LoaiTinhLuong === 'THEO_GIO' ? 'Theo giờ' : bl.LoaiTinhLuong === 'THEO_BN' ? 'Theo BN' : bl.LoaiTinhLuong;
                    ...
                    html += '<tr>' +
                        ...
                        '<td class="text-center">' + parseFloat(bl.SoGioLam || 0).toFixed(1) + '</td>' +
                        '<td class="text-center">' + parseFloat(bl.SoGioTangCa || 0).toFixed(1) + '</td>' +
